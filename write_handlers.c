 ...
 * @ind: Index at which the number startson the buffer
 * @buffer: Buffer
 * @flags: Flags
 * @width: Width
		...
		int length, char padd, char extra_c)
{
	...
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0' &&
			width == 0)
		return (0); /* printf(".0d", 0) no chr is printed */
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
	...
	if (width > length)
	{
		...
		if (flags & F_Minus && padd == '')
			/* Asign extra chat to left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[1], i - 1) +
					write(1, &buffer[ind], i - 1));
		}
...
{
	/**
	* The number is stored at the buffer's right
